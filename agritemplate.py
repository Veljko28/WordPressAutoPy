...
from slika_agri import getImage, resizeImage, removeImageBorder, imageClear
import time
import sys
import re
import os
...
# SAMO JEDAN U KATEGORIJI
# https://shop.agromarket.rs/sejalice/13357-rucna-sejalica-vhs-10
# https://shop.agromarket.rs/masine-za-zaprasivanje/37722-masina-za-zaprasivanje-mini-termal-fogger
...
def getInfo(link):
    driver.get(link)
    try:
        name = driver.find_element_by_xpath('/html/body/div[1]/main/div[4]/div[1]/div/div/div[1]/div[2]/div[1]/div[1]/div[3]/h1/span')
        main_title = name.get_attribute('innerHTML')
        try:
            table = driver.find_element_by_xpath('/html/body/div[1]/main/div[4]/div[1]/div/div/div[1]/div[2]/div[1]/div[4]')
        ...
        time.sleep(5)
        try:
            # spec = driver.find_element_by_xpath('/html/body/div[1]/main/div[4]/div[2]/div/div/div/div[1]/div/table')
            # tags = spec.find_elements_by_tag_name('a')
            # for tag in tags:
            #     driver.execute_script("arguments[0].setAttribute('href','')", tag)
            # ActionChains(driver).move_to_element(spec).perform()
            # specifikacija = spec.get_attribute('outerHTML')

            specifikacija = ""
            try:
                img = driver.find_element_by_xpath('/html/body/div[1]/main/div[4]/div[1]/div/div/div[1]/div[1]/div/div/div[1]/div/div[1]/div/div/div[1]/div/img')
                imageName = getImage(img.get_attribute('src'), driver)
                # resizeImage(imageName)
            except:
                img = driver.find_element_by_xpath('/html/body/div[1]/main/div[4]/div[1]/div/div/div[1]/div[1]/div/div/div[2]/div[1]/div[1]/div[1]/div/div/div/img')
                imageName = getImage(img.get_attribute('src'), driver)
...
def wordpress_actions(name, table_str, imageName, specifikacija):
    ...
    imagePath = "C:\\Users\\Sasha\\Desktop\\Artisoft\\pyTest\\MoneyMaker\\agrimarket\\" + imageName
...
def template(section, pages):
    hrefs = []
    for i in range(pages):
        driver.get(section+'page-'+str(i))
        time.sleep(3)
        list = driver.find_elements_by_class_name('item-data')
        for j in range(1, len(list)+1):
            try:
                tag = driver.find_element_by_xpath('/html/body/div[1]/main/div[3]/div/div[2]/div[2]/div[2]/div/div['+str(j)+']/div/div/div[3]/div[5]/a')
                hrefs.append(tag.get_attribute('href'))
            except:
                print("Failed to add one item")
